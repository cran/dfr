fit_path <- function(X,y,groups, groupIDs, type, lambda_path, alpha, intercept, pen_var_org, pen_grp_org, X_scale, X_center, y_mean, wt, wt_per_grp,
  ...
  for (lambda_id in 1:path_length){
    pen_var = alpha*lambda_path[lambda_id]*pen_var_org
    pen_grp = (1-alpha)*lambda_path[lambda_id]*pen_grp_org
    # -------------------------------------------------------------
    # run ATOS
    # ------------------------------------------------------------- 
    ...
    if (type == "logistic" & intercept){
      fit_out = run_atos_log_inter(X,y,num_obs, num_vars, groups, groupIDs, backtracking, max_iter, max_iter_backtracking, tol, x0 = warm_x, u = warm_u,
                            pen_var, pen_grp, f, f_grad, mult_fcn, crossprod_mat,verbose=FALSE)
    } else { # run SGL
      fit_out = run_atos_sgl(X,y, groups, groupIDs, pen_var, pen_grp, x0 = warm_x, u = warm_u, wt_per_grp, num_vars, num_groups,
...
fit_one <- function(X,y,groups, groupIDs, type, lambda, alpha, intercept, pen_var_org, pen_grp_org, x0=rep(0,ncol(X)), u=rep(0,ncol(X)), X_scale, X_center, 
  ...
  # weights
  pen_var = alpha*lambda*pen_var_org
  pen_grp = (1-alpha)*lambda*pen_grp_org

  # -------------------------------------------------------------
  # run ATOS
  # ------------------------------------------------------------- 
  if (type == "logistic" & intercept){ # doesn't work
    fit_out = run_atos_log_inter(X,y,num_obs, num_vars, groups, groupIDs, backtracking, max_iter, max_iter_backtracking, tol, x0, u,
                          pen_var, pen_grp, f, f_grad, mult_fcn, crossprod_mat,verbose)
  } else { # run SGL
    fit_out = run_atos_sgl(X,y, groups, groupIDs, pen_var, pen_grp, x0, u, wt_per_grp, num_vars, num_groups,
