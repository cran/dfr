...
dfr_adap_sgl.cv = function(X,y,groups, type = "linear", lambda="path", path_length = 20, nfolds=10, alpha = 0.95, gamma_1 = 0.1, gamma_2 = 0.1, backtracking = 0.7, max_iter = 5000, max_iter_backtracking = 100, tol = 1e-5, min_frac = 0.05, standardise= "l2", intercept = TRUE, error_criteria = "mse", screen=TRUE, verbose = FALSE, v_weights=NULL, w_weights=NULL){
  out = general_fit_cv(X, y, groups, gen_path_adap_sgl, type, lambda, path_length, nfolds, alpha, 
                      backtracking, max_iter, max_iter_backtracking, tol, min_frac, standardise, intercept, v_weights, w_weights, 
                      error_criteria, screen, verbose, gamma_1, gamma_2, model = "asgl")
  return(out)
}